...
_estado = { 
    ...
    "red_desconectada_desde": None,
    "red_ultimo_recordatorio": None,
} 
...
def _red(lectura):
    eventos = []
    extra = lectura["extra"]
    evento_conexion = extra.get("evento_conexion")
    ahora = time.monotonic()

    # LABORATORIO: los eventos de conexion se disparan por flanco.
    if evento_conexion == "red_desconectada":
        _estado["red_desconectada_desde"] = ahora
        _estado["red_ultimo_recordatorio"] = ahora
        eventos.append(("red_desconectada", {"conectada": False}))
    elif evento_conexion == "red_conectada":
        _estado["red_desconectada_desde"] = None
        _estado["red_ultimo_recordatorio"] = None
        eventos.append(("red_conectada", {"conectada": True}))

    # LABORATORIO: recordatorio por tiempo mientras la red continua caida.
    if extra.get("conectada") is False and _estado["red_desconectada_desde"] is not None:
        ultimo = _estado["red_ultimo_recordatorio"]
        if ultimo is not None and ahora - ultimo >= config.RED_RECORDATORIO_SEGUNDOS:
            eventos.append(("red_sigue_desconectada", {
                "segundos": round(ahora - _estado["red_desconectada_desde"], 1)
            }))
            _estado["red_ultimo_recordatorio"] = ahora

    eventos += _histeresis(
        "red", lectura["valor"], config.RED_PICO_KBS, config.RED_CALMA_KBS,
        "red_pico", "red_calma"
    )
    return eventos
